...
**S1: 数据准备（串行）**：
- `futures-datatech`: 数技源。运行通道突破扫描 `scan_all.py`（默认channel_breakout）产出 signal_summary 汇总；L1-L4 由观澜 `run_l1l4_scan.py` 产出、factor_timing 由探源 `run_factor_timing_scan.py` 产出。**不做分析，不下结论**
...
- `quant-daily`: 数技源通道突破数据管道（scan_all.py 默认 channel_breakout → full_scan_summary_{date}.json）
