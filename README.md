...
一套 **11-Agent 多角色交叉质询的 CTA 决策系统**。基于 LangGraph 构建，实现按需并行数据源、PostgreSQL OLTP+OLAP 混合存储、独立 CLI/FastAPI 入口。
...
- **11-Agent 辩论制衡** — 数技源/闫判官/链证源/观澜/探源/读心/多头分析员/空头分析员/风控明/明鉴秋/品藻，边界钉死不越界
- **5 层鲁棒防线 (L1-L5)** — 产出校验→熔断降级→信号门禁→路径发现→健康自检，各 Agent 独立超时降级
- **自进化闭环** — 多维度立体评估：品藻实时质检 + T+1 回测验证 + APM-CS 五轴评分卡(D1-D5) + 解码质量监控(D3) + 输出质量审计(D6)，覆盖辩论、解码、工具调用、输出全链路
...
11 个核心 Agent 各司其职，**不越界、不重叠**（另有 2 个辅助评估 Agent：副裁官、独立裁官）
...
| 品藻 | 辩论输出质检 + Schema校验 + 内容安全过滤 + 报告排版核验 | 不修改裁决内容 |
...
│  │  - D2 Acuity: 信号-噪音辨识力 (Spearman 秩相关)         │      │
│  │  - D3 Composure: 波动率镇定度 (stop~ADX 回归)           │      │
│  │  - D4 Discipline: 规则遵守度 (RuleChecker)               │      │
│  │  - D5 Reliability: 闭环完成率（剔除陈旧失败）            │      │
