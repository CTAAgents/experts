...
# ── 路径自举：确保 futures_data_core（§1 收编版，含 indicators.trend_maturity）
#    从本 FDT 专家包解析，而非可能被 sys.path 优先命中的其他副本（如 D:\FDT2 旧版）──
# 文件位于 .../futures-debate-team/skills/technical-analysis/scripts/l1l4_scoring.py
# 向上 4 级目录即为 FDT 专家包根 futures-debate-team（含 futures_data_core/）
_FD_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
if _FD_ROOT not in sys.path:
    sys.path.insert(0, _FD_ROOT)

from typing import Dict, Optional
import math
from futures_data_core.indicators.trend_maturity import assess_trend_maturity
...
def score_L1_germination(
    tech: dict, sym: dict, is_bull: bool, kline_closes: list = None, term_basis: dict = None
) -> dict:
...
def score_L4_confirmation(tech: dict, sym: dict, is_bull: bool, days_since_breakout: int = 0) -> dict:
...
def score_veto_dimension(tech: dict, sym: dict, is_bull: bool, term_basis: dict = None) -> dict:
...
def _determine_direction(tech: dict, sym: dict, score_direction: int = 0) -> bool:
    ...
    if (
        ...
        and mdi is not None
        and pdi < mdi
        and roc10 is not None
        and roc10 < 0
        and bb_middle
        and last_price
        and last_price < bb_middle
    ):
        return False
...
def calculate_composite_score(
    tech: dict, sym: dict, score_direction: int = 0, kline_closes: list = None, term_basis: dict = None
) -> dict:
...
def compute_maturity_penalty(l1: int, l2: int, l3: int, l4: int, rsi: float, adx: float) -> tuple:
    ...
    if rsi < 20 or rsi > 80:
        penalty += 12
        reasons.append(f"RSI极端({rsi:.0f}, -12)")
    elif rsi < 25 or rsi > 75:
        penalty += 8
        reasons.append(f"RSI超买/超卖({rsi:.0f}, -8)")
    elif rsi < 30 or rsi > 70:
        penalty += 4
        reasons.append(f"RSI偏高({rsi:.0f}, -4)")

    # ADX成熟度罚分
    if adx > 70:
        penalty += 6
        reasons.append(f"ADX极强(趋势末端, {adx:.0f}, -6)")
    elif adx > 55:
        penalty += 3
        reasons.append(f"ADX强趋势(偏晚, {adx:.0f}, -3)")
    elif adx < 15:
        penalty += 4
        reasons.append(f"ADX震荡(信号不可靠, {adx:.0f}, -4)")

    # L4确认层罚分（L4满分说明趋势已完成确认阶段）
    if abs_l4 >= 16:
        penalty += 4
        reasons.append(f"L4确认层满分(-4)")
    elif abs_l4 >= 12:
        penalty += 2
        reasons.append(f"L4确认层偏高(-2)")

    # L3结构层满分的趋势也偏晚
    if abs_l3 >= 15:
        penalty += 2
        reasons.append(f"L3结构层满分(-2)")
