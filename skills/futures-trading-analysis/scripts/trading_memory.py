...
def record_decision(symbol: str, direction: str, entry_price: float, reason: str, confidence: float = 0.0) -> str:
    """记录本次辩论决策。
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"symbol": symbol.upper(), "records": records}, f, ensure_ascii=False, indent=2)
...
def build_reflection_prompt(symbol: str) -> str:
    ...
    if closed:
        ...
        for r in closed[:3]:
            ...
            parts.append(
                f"  {pnl_icon} {r.get('date', '?')} {dir_label}@{r.get('entry_price', 0)} → {r.get('exit_price', 0)} PnL{pnl:+.1f}%"
            )
