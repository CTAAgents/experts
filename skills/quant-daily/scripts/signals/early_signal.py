...
def detect_volatility_expansion(atr_values: List[float], lookback: int = 20, expansion_threshold: float = 1.5) -> dict:
    """检测波动率突破（ATR收缩后扩张）。
...
def detect_open_interest_change(
    open_interests: List[int], prices: List[float], lookback: int = 5, oi_change_threshold: float = 0.05
) -> dict:
    """检测持仓量变化（增仓突破）。
...
def detect_short_term_momentum(closes: List[float], period: int = 5) -> dict:
    ...
    if len(closes) < period + 1:
        return {"rsi_5": 50, "ma_5": 0, "price_vs_ma5": "at", "momentum": "neutral", "signal_strength": "weak"}
...
def generate_early_signal_alert(early_signals: dict, product_id: str, product_name: str) -> str:
    ...
    direction_emoji = (
        "🟢"
        if early_signals["early_direction"] == "bullish"
        else "🔴"
        if early_signals["early_direction"] == "bearish"
        else "⚪"
    )
    strength_emoji = (
        "🔥"
        if early_signals["overall_signal_strength"] == "strong"
        else "⚡"
        if early_signals["overall_signal_strength"] == "moderate"
        else "💤"
    )
...
def detect_oi_triangle(
    prices: List[float], open_interests: List[int], volumes: List[float], lookback: int = 10
) -> dict:
    """检测OI三角信号（期货专属，最早最核心）。
