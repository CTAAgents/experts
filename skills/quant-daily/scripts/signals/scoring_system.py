# -*- coding: utf-8 -*-
"""100分制多维打分系统 v2.17 — L1-L4四层架构（保留v2.12全部因子）
...
- L2 量价领先维度（25分, v2.17）：Vortex(7分)、CCI(5分)、Supertrend(7分)、HMA(6分)
- L3 价格结构维度（25分, 原15分）：RSI健康区(8分)、DMI方向(4分)、前高突破(3分)
...
def calculate_composite_score(tech: dict, sym: dict, score_direction: int = 0,
    ...
    # v2.17 权重重分配: 40/25/25/10
    WL1, WL2, WL3, WL4 = 40, 25, 25, 10  # v2.17: L4 15→10
    l1_scaled = round(min(l1_raw['score'], 40) * WL1 / 40.0)  # v2.17: clamp 40→40
    l2_scaled = round(l2_raw['score'] * WL2 / 25.0)  # v2.17: internal max 15→25
    l3_scaled = round(l3_raw['score'] * WL3 / 25.0)  # v2.17: internal max now 25
