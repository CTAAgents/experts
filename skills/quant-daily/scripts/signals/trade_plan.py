...
def generate_trade_plan(
    ...
    # G34 Turtle：tech_data.turtle_units 单位预算（默认 1，零回归）
    # 1 单位=基线(1.0x)，每多 1 单位 +0.25x（保守，不与 G32 重复缩放）
    _turtle_units = int((tech_data or {}).get("turtle_units", 1) or 1)
    turtle_factor = 1.0 + max(0, _turtle_units - 1) * 0.25
    pos = round(min(max(base * pos_mult * rr_mult * vol_mult * adx_mult * vol_scale * turtle_factor, 2.0), 10.0), 1)
