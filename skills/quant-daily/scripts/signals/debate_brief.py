...
def _extract_risk_input(l_entry: dict, f_entry: dict) -> dict:
    ...
    # 置信度：技术分析评分的cons(一致性) + 因子择时的vote_confidence 加权
...
def build_judge_brief(symbol_entry: dict, debate_score: dict, risk_input: dict = None) -> dict:
    ...
    """
    l = symbol_entry.get("signal", {})
    f = symbol_entry.get("factor", {})
    ...
    l_summary = f"{l_dir}(总分{l_total:+.0f}"
    l_summary += f", ADX{l_adx})" if l_adx else ")"
    f_summary = f"因子{f_dir}(总分{f_total:+.0f})"
...
def _extract_signal(entry: dict) -> dict:
    """提取信号原始数值"""
    return {
...
def build_signal_summary(scan_path: str, period_fitness_path: str = None) -> dict:
    """
    构建信号汇总表。
    ...
    """
    scan_data = _load_json(scan_path)
    all_ranked = scan_data.get("all_ranked", [])
    all_symbols = [e["symbol"] for e in all_ranked]
...
def build_html(summary: dict) -> str:
        ...
        [
            {
                "sym": s["symbol"],
                "name": s["name"],
                "total": s["signal"]["total"],
                "dir": s["signal"]["direction"],
                "grade": s["signal"]["grade"],
                "adx": s["signal"]["adx"],
                "rsi": s["signal"]["rsi"],
            }
    ...
    html = f"""<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>信号汇总 — quant-daily</title>
<style>
...
<div class="hd"><h1>信号汇总 — quant-daily</h1>
<div class="m"><span>{meta["total_symbols"]}品种</span><span>多头{meta.get("bull",0)}/空头{meta.get("bear",0)}</span><span>纯数据、不做判断</span></div></div>
<div class="st">
<div class="sc bl"><div class="n">{meta.get("bull",0)}</div><div class="l">多头</div></div>
<div class="sc b"><div class="n">{meta.get("bear",0)}</div><div class="l">空头</div></div>
</div>
<p style="color:#6b7280;font-size:11px;margin-bottom:10px">quant-daily 仅输出原始信号数值</p>
<table id="tbl"><thead><tr>
<th onclick="sortBy(0)">品种</th>
<th onclick="sortBy(1)">总分</th><th onclick="sortBy(2)">方向</th><th onclick="sortBy(3)">等级</th><th onclick="sortBy(4)">ADX</th><th onclick="sortBy(5)">RSI</th>
</tr></thead><tbody id="tb"></tbody></table>
<script>
var DATA={rows_json};
function _v(d,c){{var a=[d.sym,d.total,d.dir,d.grade,d.adx,d.rsi];return a[c];}}
...
var dc=d.dir==='bull'?'#22c55e':(d.dir==='bear'?'#ef4444':'#6b7280');
h+='<tr><td style="font-weight:700">'+d.sym+'</td>';
h+='<td style="color:'+dc+'">'+(d.total>0?'+':'')+d.total+'</td><td style="color:'+dc+'">'+d.dir+'</td><td>'+d.grade+'</td><td>'+d.adx+'</td><td>'+d.rsi+'</td></tr>';}}
...
def select_debate_symbols(
    ...
    for s in symbols:
        ...
        quick_summary = f"技术分析评分{l_dir_cn}(总分{l_total:+.0f}, ADX{l_adx_val:.1f}) vs 因子{f_dir_cn}(总分{f_total:+.0f})"
        conflict = (l_dir != f_dir) and (l_dir != "neutral" and f_dir != "neutral")
    ...
    for item in divergence:
        ...
        item["reason"] = (
            ...
            if item.get("left_side_reversal")
                ...
                f"方向分歧: L1L4={item['l1l4_dir']}({item['l1l4_total']:+.0f}) vs 因子={item['factor_dir']}({item['factor_total']:+.0f})"
            )
        )
        # 添加历史调整值
        if history_feedback:
    ...
    for item in consensus_bear:
        if item["chain"] not in covered_chains and len(candidates) < min_count + 5:
            ...
            item["reason"] = (
                f"双空共识: L1L4({item['l1l4_total']:+.0f}) + 因子({item['factor_total']:+.0f}) | 链:{item['chain']}"
            )
            if history_feedback:
                try:
                    from debate import history as dh
...
if __name__ == "__main__":
    ...
    parser.add_argument("l1l4_path", help="技术分析评分 策略 JSON 路径")
    parser.add_argument("factor_path", help="因子择时 策略 JSON 路径")
    ...
    print(f"  技术分析评分: {meta['l1l4_bull']}多头 / {meta['l1l4_bear']}空头")
