...
class FreshnessRecord:
    ...
    def __repr__(self) -> str:
        return (
            f"<FreshnessRecord {self.variety}: status={self.status}, "
...
class DataFreshnessMonitor:
    ...
    def __init__(self, data_dir: str = None):
        ...
        # 今天的记录文件
        today = date.today()
        self._today_file = os.path.join(self.data_dir, f"freshness_{today.isoformat()}.json")
    ...
    def get_sla_history_point(self, days: int = 7) -> Dict:
            ...
            points.append(
                {
                    ...
                    "freshness_score": r.get("freshness_score", self._calc_score_from_report(r)),
        ...
        return {
            ...
            "trend": "up"
            if len(points) >= 2 and points[0]["sla_coverage_pct"] > points[-1]["sla_coverage_pct"]
            else "stable",
        }
            ...
            def _generate_with_alerts():
                ...
                if report.get("is_trading_day") and report.get("deadline_passed"):
                    ...
                    for v in outages[:5]:  # 最多报5个，避免刷屏
                        _am.alert_sla_violation(v, "freshness", f"SLA 截止{report.get('sla_deadline', '?')} 后仍未采集")
...
def record_data_fetch(
    variety: str, data_source: str = "unknown", success: bool = True, count: int = 0, error: str = ""
):
...
def main():
    ...
    if cmd == "report":
        ...
        print(f"SLA 合规率: {report['sla_coverage_pct']}%")
        print(
            f"[OK] 新鲜: {report['fresh']}/{report['total_varieties']} | "
        ...
        if not retries:
            ...
            for r in retries:
                print(
                    f"  - {r['variety']}: {r['last_error'][:60]} (已重试 {r['retry_count']}/{r['remaining_retries']})"
                )
        ...
        print(
            "Usage: python data_freshness_monitor.py [report|score|status <variety>|retry|history [days]|clean [days]]"
        )
