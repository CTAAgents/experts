    ...
    try:
        ...
        if isinstance(resp, dict) and resp.get('success'):
            ...
            if len(valid) >= 50:
                ...
                print(f'  ✅ {sym} ({TARGET_NAMES[sym]}): {len(valid)} bars')
            else:
                print(f'  ⚠️ {sym}: only {len(valid)} valid bars (need ≥50)')
        else:
            print(f'  ❌ {sym}: data fetch failed - {resp}')
    ...
    try:
        ...
        if sc['reasons']:
            for r in sc['reasons'][:8]:
                notes.append(r)
        if layer_warnings:
            notes.append(f'⚠️ 层间矛盾: {"; ".join(layer_warnings)}')
        if not veto_pass:
            notes.append(f'❌ 否决项触发(veto={sc["veto_score"]})')
        else:
            notes.append(f'✅ 否决项通过(veto={sc["veto_score"]})')
        ...
        print(f'  ✅ {sym} ({TARGET_NAMES[sym]}): {direction} | 总分{total:.0f} | {grade} | {stage}')
