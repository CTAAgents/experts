...
| 2 | **Spawn类型**：**所有文件产出Agent**（P1.5链证源/P3研究员/P4辩手/P5裁决/策执/风控/一致性裁判）统**必须用 `subagent_type: "general-purpose"`**，不可用expert subagent_type。Expert spawn时Tools为空(无Write)→文件写不出去 | D05 |
...
## 🔴 辩论缺员降级与 LLM 工程化（A+B 加固 · 2026-07-14）

### A3 缺员降级（degrade-on-failure）
...
- 缺 `p5_trading_plan` / `p5_risk_review` → 该品种标 `仅裁决`，仍进报告。
- 主管 spawn 完毕后若仍有缺产出品种，运行
  `python scripts/run_debate.py repair --scan <json> --workspace <ws>`
  生成 `repair_plan_*.json` 补辩计划，重 spawn 即可（B4 失败重排）。
...
角色默认值：观澜 0.1 / 多头分析员·空头分析员 0.4 / 闫判官·一致性裁判 0.0 / 策执远 0.5 / 风控明 0.2，
...
每个品种循环执行以下完整链条（品种之间可并行）:
  ...
  闫判官(裁决) → Read(多头分析员+空头分析员) → Write(p5_judge_{sym}.json)
...
```bash
python extract_knowledge.py ingest   --symbol {sym}   --pro p4_bullish_{sym}.json   --con p4_bearish_{sym}.json   --judge p5_judge_{sym}.json   --plan p5_trading_plan_{sym}.json   --bypass
```
...
**Phase门禁检查（D03）**：若任一缺少 `p4_bullish_{sym}.json` / `p4_bearish_{sym}.json` / `p5_judge_{sym}.json` 文件 → **拒绝生成报告**。
