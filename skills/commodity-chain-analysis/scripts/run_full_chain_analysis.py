...
用法: python run_full_chain_analysis.py [YYYY-MM-DD]
若不传日期，自动使用今天日期。
"""
...
def main():
    ...
    for chain_name in sorted(chain_data.keys()):
        ...
        strategy_report.append(
            f"- **品种数量**: {data['member_count']} | BUY: {data['buy_count']} | SELL: {data['sell_count']}"
        )
        ...
        for m in data["members"]:
            ...
            strategy_report.append(
                f"| {m['symbol']}({m['name']}) | {m['price']} | {m['l1l4_direction']} | {m['l1l4_total']} | {m.get('z_score', 'N/A')} | {cross_tag} |"
            )
        strategy_report.append("")
    ...
    for chain_name, data in chain_data.items():
        for m in data["members"]:
            ...
            if ci:
                state = "default"
                analysis_report.append(
                    f"| {m['symbol']} | {chain_name} | {state} | {ci['dominant']} | {ci['reason']} |"
                )
    analysis_report.append("")
    ...
    for chain_name in sorted(chain_data.keys()):
        data = chain_data[chain_name]
        analysis_report.append(
            f"| {chain_name} | {data['consistency_pct']}% | {data['trend']} | {data['buy_count']}/{data['sell_count']} |"
        )
    analysis_report.append("")
    ...
    for chain_name in sorted(chain_data.keys()):
        data = chain_data[chain_name]
        print(
            f"{chain_name:12s} | {data['trend']:12s} | 品种{data['member_count']} | 一致性{data['consistency_pct']}% | BUY={data['buy_count']} SELL={data['sell_count']}"
        )
