...
def run_pipeline(symbols_data: list = None):
    ...
    for sym in top10_symbols:
        ...
        if chain_name:
            ...
            if chain_name not in chain_top_symbols or abs(sym.get("score", 0)) > abs(
                chain_top_symbols[chain_name].get("score", 0)
            ):
    ...
    for chain_name, symbol in chain_top_symbols.items():
        try:
            ...
            print(
                f"  ✓ {chain_name} ({pid}): {decision.get('verdict', 'HOLD')} (信号:{symbol.get('direction', 'HOLD')}, 得分:{symbol.get('score', 0):.0f})"
            )
        except Exception as e:
    ...
    for chain_name, debate_data in debate_results.items():
        try:
            ...
            if leader_symbol:
                ...
                # 风险主管裁决
                risk_decision = {"aggressive": aggressive, "conservative": conservative, "neutral": neutral}
