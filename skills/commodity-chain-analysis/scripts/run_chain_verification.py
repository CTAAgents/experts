        ...
        result.append(
            {
                ...
                "trend": {
                    "score": s["score"] if s["direction"] == "BUY" else -s["score"],
...
def main():
    ...
    for s in SYMBOLS_DATA:
        ...
        if chain and chain in chain_results:
            ...
            print(
                f"  {pid_upper}({s['product_name']}): {chain}趋势={chain_trend} | {align_str} | 同向{chain_members_same_dir}/{chain_total}={chain_ratio:.0%}"
            )
        else:
            verification_results[pid_upper] = {
                "chain": chain or "未知",
    ...
    for chain_name, symbols_in_chain in chain_groups.items():
        ...
        for pid_a, pid_b in high_corr_pairs:
            ...
            if a_data["direction"] != b_data["direction"]:
                print(
                    f"    {pid_a.upper()}({a_data['direction']}) vs {pid_b.upper()}({b_data['direction']}) — 方向不同，无需冗余"
                )
                continue
    ...
    for pid, info in final_output.items():
        ...
        print(
            f"{pid:>6} | {dir_str:>4} | {score_str:>4} | {info['chain']:<12} | {term_str:<12} | {trend_str:<10} | {align_str:>4} | {red_str:>5} | {z_str:>5}"
        )
