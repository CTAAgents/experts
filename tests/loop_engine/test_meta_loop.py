"""
tests/loop_engine/test_meta_loop.py — L1 Meta-Loop 测试
...
from loop_engine.meta_loop import (
    BootstrappingChain,
...
from loop_engine.seed_pool import SeedPool
...
class TestFactorPoolManager:
    ...
    def test_add_entry(self, tmp_factor_pool_path):
        ...
        mgr = FactorPoolManager(tmp_factor_pool_path)
        mgr.load_or_init()
        from loop_engine.contracts import FactorPoolEntry
...
class TestMetaLoop:
    ...
    def test_circuit_breaker_on_consecutive_low_quality(
        ...
        # 构造一个所有候选都被拒绝的场景
        # 通过让所有候选名都与现有种子重名
        from loop_engine.meta_loop import BootstrappingChain
        from loop_engine.contracts import EconomicLogic, FactorSignature, SeedCandidate
