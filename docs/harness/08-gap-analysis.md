...
| **G31** | 缺时间序列动量 TSMOM（理论基石） | 现代 CTA 趋势跟踪两基石之一（Moskowitz-Ooi-Pedersen 2012）缺失，FDT 仅有跨品种截面信号，无"过去 N 月收益符号定多空"的时间序列维度 | 中 | 规划：指标管线新增 `tsmom_1m/3m/6m/12m` 收益字段（OHLC 派生，零外部源）；`trend_following_strategy` 增 `_score_tsmom`（多窗口合成降噪）；`settings.py` 阈值。**版本 v8.0.4** | ⏸ 规划中（G30 收口后启动） |
| **G32** | 缺波动率目标化 Vol Targeting | 现代 CTA 趋势跟踪两基石之二（AQR 标配）缺失，头寸未按实现波动率倒数缩放 → 高波动期过度暴露 | 中 | 规划：**落点=执行/风险 overlay 层**（非信号评分层，与 G34 Turtle N 单位统一）；数据=实现波动率（收益派生，依赖 G31）。`StrategyFusion` 或执行层按目标波动（如 10% 年化）缩放 raw_score/头寸。**版本 v8.0.5** | ⏸ 规划中（依赖 G31） |
| **G33** | 缺 Dual Thrust 日内突破 | 期货圈实盘高频（Tony Crabel 1990），纯价量，FDT 当前无日内突破型趋势工具 | 低 | 规划：指标管线新增前一日 H/L/C 区间字段；`trend_following_strategy` 增 `_score_dual_thrust`；`settings.py` 阈值。**版本 v8.0.6** | ⏸ 规划中（G30 收口后启动） |
| **G34** | 缺 Turtle 完整系统 | 行业标杆（Dennis/Eckhardt 1983），FDT 已有 DC20/55+ATR 但缺 N 单位头寸 + 金字塔加仓 + 2 单位退出完整规则 | 中 | 规划：复用 DC20/55+ATR（G30 退出工具 Supertrend/Chandelier 可复用）；**金字塔加仓与 N 单位头寸确认落执行/头寸管理层（非信号评分层）**。**版本 v8.0.7** | ⏸ 规划中（G30/G32 收口后启动） |
...
**当前成熟度：8 维全 5/5**。G1-G24 关闭，**G25/G26/G27/G28/G29/G30 已关闭**（OI 全线补全 + 扫描 4x 提速 + 多因子因子接入 + 策略暂停开关 + 宏观 rate/pmi 真实公开源接入 + 趋势跟踪指标衍生 Keltner/Supertrend/SAR/Chandelier/MACD 五子策略）；**G31-G34 规划中**（TSMOM 时间序列动量 / Vol Targeting 波动率目标化 / Dual Thrust 日内突破 / Turtle 完整系统）。CTA 策略覆盖 7/7（4 活跃 + 3 暂停）· pipeline 默认模式 · v8.0.3
...
**所有差距已关闭**（2026-07-15 13:30 — G25+G26+G27+G28+G29 全部关闭）：G16 测试修复 + G14 版本迁移 + G17/G18 文档整顿 + G25 OI 数据全线补全（类型定义→三采集器→消费端闭环）+ G26 全量扫描并发提速（tdx 超时 15→3s + scan_all ThreadPool 4 工）+ G27 多因子因子接入（warrant 真实全量源已通、inventory/capacity 惰性待源）+ G28 策略持久化暂停开关（config DISABLED_STRATEGIES 暂停多因子/AI-ML/事件驱动，代码零删除）+ G29 宏观 rate/pmi 因子接入真实公开源（东方财富宏观数据中心 PMI/LPR1Y，连接器+ctx 注入+因子评分替换硬 0，实盘验证本沙箱可直连、PMI 50.3/LPR1Y 3.0% grade=DAILY）。FDT 全系统达成 Harness 8 维满分驾驭工程标准。
