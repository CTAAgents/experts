...
> - **v9.22.0 (RHI 完整落地)**: `evolution_graph.py` 路由链新增 `rhi` 分支（improve→calibrate→evolve→rhi→ml→complete），`FDT_RHI=true` 环境变量开关。`scripts/rhi_global_cli.py` 全局 CLI 工具，任何项目可独立使用 RHI Harness 自优化。`D:\HarnessStarterKit\scripts\rhi_global_setup.py` 独立部署脚本，`docs/harness-templates/` 镜像同步更新。
