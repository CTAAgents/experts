...
| v8.10.0 | 2026-07-18 | **L1/L3 Loop Engineering（Phase 2+3）**：① L1 Meta-Loop — 每日感知因子池缺口 + Bootstrapping Agent 链自动补种（`loop_engine/meta_loop.py`，51 测试）；② L3 Portfolio Loop — 信号合成→正交化→组合构建→衰减检验→注入 FDT（`loop_engine/portfolio_loop.py`，34 测试）；③ loop_engine 累计 181 测试全绿（96 L2 + 51 L1 + 34 L3）。 |
...
| v8.9.3 | 2026-07-18 | **L2 因子演化循环（Loop Engineering Phase 1 落地）**：① 新增 `loop_engine/` 包（12 模块）：`factor_program.py`（安全沙箱执行器 + `_safe_import` 白名单 + `secrets.token_hex` 因子 ID 唯一性）、`state.py`（演化状态管理 + 先写主文件再镜像 backup 一致性保证）、`evolution_loop.py`（L2 主循环 + 经验链轨迹 + 四重熔断）、`contracts.py`/`elite_archive.py`/`verifier_protocol.py` 等；② 新增 `tests/loop_engine/`（7 测试文件 / **96 个测试用例全绿**）；③ `scheduler/tasks.py` 新增 `l2_evolution_loop` 任务（subprocess.run 调用 `python -m loop_engine.evolution_loop --once`，4h timeout，环境变量可配置）；④ `scheduler/triggers.py` 新增第 14 个触发器（每晚 20:00 TimeTrigger）；⑤ 12 个种子因子从现有 trend_following/mean_reversion/多因子策略中提取；⑥ Verifier 协议锁定不可修改、四重熔断（Token>2x / 连续 3 代 IC<0.01 / 失败率>90% / 状态 24h 未更新）；⑦ pyproject.toml 同步更新：version 8.9.2→8.9.3、testpaths 添加 `tests/loop_engine`、packages.find 添加 `loop_engine*`、addopts 添加 `--cov=loop_engine`。版本号 bump 8.9.2→8.9.3 |
