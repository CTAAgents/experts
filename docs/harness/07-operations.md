...
| **v9.25.0** | 2026-07-24 | **记忆系统全面重构** — MemoryManager 统一入口消灭 26 处散落直写；6 个僵尸脚本/目录清除（~1300 行死代码）；检索断层修复（`VectorRetriever` 整合 VectorMemory）；维护自动化（`Cleaner`/`Archiver`/`Decay`/`Checker`）；`check_memory_gaps.py` 最终实现；master_graph 每日 04:00 自动维护。详见 [memory-system-overhaul.md](../designs/memory-system-overhaul.md)。 |
