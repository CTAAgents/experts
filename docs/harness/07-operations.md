...
| 版本 | 日期 | 里程碑 |
|:-----|:-----|:-------|
| v9.2.0 | 2026-07-18 | **Loop Engineering 剥离（因子自演化移出 FDT 系统）**：① 删除 `loop_engine/` 代码目录（16 模块）和 `tests/loop_engine/` 测试目录（11 文件）；② 删除 `docs/harness/11-loop-engineering.md`、`docs/LOOP_ENGINEERING_OPS_MANUAL.md`、`docs/LOOP_ENGINEERING_INTEGRATION_PLAN.md` 三篇文档；③ 更新 5 篇 Harness 文档（02-lifecycle 删除 4a/4b 节、06-testing 删除第 11 节、07-operations 删除 v8.10.0/v8.9.3、08-gap-analysis 删除 G78/G80/G81、01-architecture 不受影响）；④ 更新 `scheduler/tasks.py`（删除 l2_evolution_loop/l1_meta_loop/l3_portfolio_loop 三个任务）、`scheduler/triggers.py`（删除 3 个触发器）、`pyproject.toml`（移除 loop_engine 引用）；⑤ 更新 `README.md` 移除 L2 特性描述和版本历史；⑥ 全量测试验证通过。版本号 bump 9.1.0→9.2.0 |
