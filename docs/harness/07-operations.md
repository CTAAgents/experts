...
| v9.19.0 | 2026-07-23 | **pipeline/runner.py 强制退役，LangGraph 为唯一辩论执行路径**：① 删除 `pipeline/runner.py`、`quality_filter.py`、`__init__.py`；② 清理 `FDT_USE_LANGGRAPH` A/B 切换机制；③ 删除 `tests/pipeline/`、`tests/fdt_langgraph/test_integration_ab.py`、`tests/quant-daily/test_quality_filter.py`；④ `fdt_daily_runner.py` 改为调用 `fdt_cli.py run`；⑤ 清理所有文档中 A/B 切换描述；⑥ G108 Task 1/6 关闭。版本号 bump 9.18.0→9.19.0 |
