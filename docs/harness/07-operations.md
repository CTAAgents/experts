...
| 位置 | 当前版本 | 格式 |
|:-----|:---------|:-----|
| `pyproject.toml` | **9.12.0** | **唯一版本源**（`bootstrap.py` 经 `scripts/fdt_paths.py:get_fdt_version()` 运行时读取） |
...
| 版本 | 日期 | 里程碑 |
|:-----|:-----|:-------|
| v9.15.0 | 2026-07-23 | **Generation Phase 4 — 解码参数反馈闭环（升温重试）**：① `enforce_structured_output.py` 新增 `retry_with_temperature_escalation()` — 结构化输出校验失败时按 `decode_config.yaml retry_config.temperature_multiplier` 逐次升温重试；② 新增 `write_retry_signal()` 信号文件机制；③ `agent_waiter.py` `_validate_agent_output` 校验失败时自动写入重试信号文件；④ 更新 `10-coding-standards.md` 运行时强制 §5 升温重试。版本号 bump 9.14.0→9.15.0 |
