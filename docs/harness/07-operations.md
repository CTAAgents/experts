...
| 版本 | 日期 | 里程碑 |
|:-----|:-----|:-------|
| v9.16.0 | 2026-07-23 | **D2/D5/D6 工程成熟度提升**：① D6 Output: `check_report_integrity` 接入 `OutputMetrics.score_output()`、`node_report` 接入 `OutputVersioning.save_output()`、`node_quality_inspect` 接入 `OutputAudit.log()`；② D2 Tool: `FdtAgentExecutor.execute()` 接入 `ToolMetrics.record_call()`；③ D5 Memory: `memory_cleaner` 增强（debate_journal 压缩+generation_metrics 清理）；④ `scheduler/tasks.py` 注册 `apm_scorecard` 定时任务。版本号 bump 9.15.0→9.16.0 |
