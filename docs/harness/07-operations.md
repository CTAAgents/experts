...
| **v9.6.4 → v9.6.5** | 2026-07-20 | **G93-G96 LangGraph 迁移全部完成** — G93: coordinator.py→graph.py Profile切换；G94: debate_protocol_v2.py→nodes.py 常量内联；G95: agent_runner.py→agents.py run_single()；G96: deploy.py INSERT 写入逻辑。3 旧文件删除，16 迁移测试通过，D2/D3/D5/D6 四维升至 ★★★★★ |
