...
| 位置 | 当前版本 | 格式 |
|:-----|:---------|:-----|
| `pyproject.toml` | **9.20.1** | **唯一版本源**（`fdt_cli.py` 经 `scripts/fdt_paths.py:get_fdt_version()` 运行时读取） |
...
| v9.20.1 | 2026-07-23 | **v9.20.0 修复不全跟进批（G110 补全 + G112-G113）**：① **质检 conditional_required 补全（G110 真正修复）** — `quality_inspector.py` `validate_verdict()` 新增条件必填字段跳过逻辑，`neutral` 方向时 `entry_price`/`stop_loss`/`target1` 不再强制；② **`evolve_agents.py` 知识萃取防崩溃（G112）** — `extract_knowledge_from_validated_verdicts()` 调用添加 `try/except` 避免 `main()` 整体崩溃；③ **`fdt_cli.py` ev_state None 保护（G113）** — `run_debate()` 中进化返回值 None 时不调用 `.get()`，输出友好提示而非 `AttributeError`；④ **FUND/TECH LLM 解析 debug 日志** — LLM 原始输出内容以 `logger.debug` 级别记录前 300 字符，便于定位解析失败根因。版本号 bump 9.20.0→9.20.1 |
