...
```
① 信号扫描：数技源 scan_all.py（channel_breakout）产出 full_scan_summary_{date}.json；观澜 TechnicalOutput（LLM推理）/ 探源 FundamentalStateVector（LLM推理）/ 读心 SentimentStateVector（LLM推理）为分析师按需能力
...
**闫判官拥有辩论调度权**：决定辩论哪些品种/产业链/方向，并 dispatch 四分析师（链证源做产业链分析、观澜做技术面分析、探源做基本面分析、读心做新闻情绪分析）。链证源/观澜/探源/读心只做各自分析、**无调度权**；四者为**平级分析师**（仅分析方向不同），彼此之间**不存在调度与被调度关系**；明鉴秋负责按闫判官指令执行 spawn 与资源管控。
...
| 读心 | `futures-news-sentiment-analyst` | 新闻情绪分析供弹（客观中立） |
...
- **分析师中立供弹**：观澜（技术面）、探源（基本面）、链证源（产业链）、读心（新闻情绪）作为客观研究员，只提供分析数据，不参与辩论。辩手只能使用分析师提供的资料，不能自行搜集数据
...
>
> **v9.11.0 变更**：新增"读心"新闻情绪分析师，通过金十 MCP（快讯/资讯/日历）和网页爬虫获取新闻情绪事件流，作为第四源与链证源/观澜/探源并行。P3 四源并行中增加 `node_sentiment()` 节点。
