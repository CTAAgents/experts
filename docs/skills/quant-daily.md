...
version: 2.15.0
agent_created: true
description: 商品期货量化分析skill v2.15.0 — TDX对齐DC20+会话感知子周期+动量识别+R0归一化+ADX移除评分(channel_breakout v1.3)。AKShare分钟→东方财富→TqSDK(15s超时保护)。scan_all 仅出通道突破信号，L1-L4/因子择时由独立 skill 产出。
---
...
- **L1-L4 原始指标** → 由 `technical-analysis` skill 的 `run_l1l4_scan.py` 独立产出（已迁出 scan_all）
- **因子择时 5 因子信号** → 由 `fundamental-data-collector` skill 的 `run_factor_timing_scan.py` 独立产出（已迁出 scan_all）
...
# L1-L4 与 因子择时已迁出独立 skill，不再由 scan_all 产出：
...
| `layered_l1l4` | （已迁出 technical-analysis skill） | ⛔ 迁出 | L1-L4 四层累加，2026-07-14 迁至 `run_l1l4_scan.py` |
