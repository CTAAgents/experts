...
> 🔴 **执行协议单一来源声明（2026-07-11 #4修复·优先级高于本文件下方所有执行类描述）**
>
> **本文件是「用户/自动化入口与概念架构」文档。** 实际的 spawn / 通信 / 降级 / L1 校验 / Spawn重试 协议，以同级技能 **[`fdt-spawn-debate`](skills/fdt-spawn-debate/SKILL.md) 为唯一权威来源**（A01 文件优先通信协议）。
>
> 本文件下方存在**历史遗留、已与当前架构冲突**的描述。自动化与人工执行时**一律以 `fdt-spawn-debate` 为准，不得执行以下模式**：
>
> | 废弃模式 | 位置参考 | 正确做法（见 fdt-spawn-debate） |
> |:--------|:--------|:-------------------------------|
> | `SendMessage` 作为主通信通道（"完成后用 SendMessage 发送给 main"） | spawn协议 / 产出 章节 | **文件优先通信**：Agent 只 Write 文件，明鉴秋 `poll_file_ready` 轮询读取（A01） |
> | `debate_team.run(...)` 编排调用 | 调度协议 / P3 编排代码 | 主循环用 `Agent(...)` 工具直接 spawn `general-purpose` 子Agent |
> | `Command(goto=下游Agent)` Handoff 直连 | 执行流程 / P3-P5 | 每阶段由明鉴秋轮询上游文件就绪后 spawn 下一跳（Supervisor 模式） |
> | `repair_phase()` 自动修复 | 容错章节 | 校验失败 → 重spawn 该Agent（最多2次，见 L1/R29 + Spawn重试 R32） |
> | `PhaseGuard` / `TeamCreate` / inbox 双写恢复 | 容错章节 | 当前为单 main 循环 + 文件通信，无团队/消息路由依赖 |
> | 置信度 "高/中/低" 裸字符串 | 各 spawn prompt | `confidence` 统一为 **0-1 数值**（#5，见 `confidence_utils.normalize_confidence`） |
...
> ⚠️ **盘前预计算缓存已于 2026-07-11 移除**：原「预计算缓存路由」(Step 4) 读取端从未接入主流程，属死缓存；辩论每次进入 P1 均实时运行 `scan_all.py`，不再有缓存命中跳过逻辑。
