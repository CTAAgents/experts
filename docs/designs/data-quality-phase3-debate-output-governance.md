...
**方案：三权分立 — 明鉴秋专注调度，新增「质监署」独立质检**

| 角色 | 定位 | 职责 | 来源 |
|:-----|:------|:-----|:------|
| **明鉴秋** | 团队主管·调度 | 选题、spawn、逐品种生命周期管理、记忆写入 | 现有角色瘦身 |
| **质监署** | 独立质检官 | Schema 校验、退回裁定、自优化指标记录、gap 登记 | **新增** |
| **文胆** | 文书 | 报告排版、HTML 生成、文件归档 | **从明鉴秋分离** |

职责转移：

```
明鉴秋(当前)          明鉴秋(新)          质监署(新)          文胆(新)
─────────────         ──────────          ─────────           ──────
选题 ✓               选题 ✓
spawn ✓              spawn ✓
汇总 ✓               汇总 ✓
...
如果保持明鉴秋一人承担，必须加以下约束防死锁：

1. **退回计数器在 state 层硬限**：`max_retries_per_symbol = 2`，LangGraph 条件边直接读 state 判断，不经明鉴秋决策
2. **质检逻辑下沉为纯函数**：`validate_argument()` 等做成无状态工具函数，明鉴秋只调用不修改规则
3. **明鉴秋自身质检**：在 `debate_results.json` 写出后做自动化 `check_report_integrity()` 检查：必需区块存在、无占位文本、文件大小合理

## 8. 更新后的实现计划

| 步骤 | 改动文件 | 内容 |
|:-----|:---------|:-----|
| 1 | `contracts/debate_quality_schema.py` | 新增 — 定义 ARGUMENT_SCHEMA / VERDICT_SCHEMA / RISK_SCHEMA TypedDict |
| 2 | `fdt_langgraph/quality_inspector.py` | 新增 — 质监署质检器，纯函数：validate_argument/validate_verdict/validate_risk |
| 3 | `fdt_langgraph/state.py` | 新增 quality_report / rework_counters / phase_timings 字段 |
| 4 | `fdt_langgraph/graph.py` | 新增质检条件边（通过/不合格/熔断三条分支） |
| 5 | `fdt_langgraph/nodes.py` | 新增 node_quality_inspect + node_report_writer（文胆） |
| 6 | `agents/` | 新增质监署角色 md + 更新明鉴秋角色 md（移除质检和报告职责） |
| 7 | `docs/harness/` | 更新 01-architecture / 02-lifecycle / 06-testing |
