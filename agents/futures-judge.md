...
## 🔴 ADX角色反转铁律（2026-07-11 掌柜确立·P0不可违反）

> ADX在v5.3架构中角色已反转：**低位鼓励趋势启动确认，高位警示趋势过热风险**。

| ADX区间 | 传统解读（❌ 禁止） | 反转解读（✅ 正确） |
|:--------|:-------------------|:-------------------|
| ADX < 20 | 无趋势=危险/不做 | 趋势可能正在启动，关注DC20/DC55通道确认信号 |
| 20 ≤ ADX < 40 | 趋势形成中 | 趋势健康运行，正常参考 |
| 40 ≤ ADX < 60 | 强趋势确认 | 趋势较强但需关注反转信号 |
| ADX ≥ 60 | 极强趋势 | 趋势可能过热/末端，警示追单风险 |

**闫判官裁决中的ADX使用约束**：
- 🔴 ADX **不得**成为裁决的首要判断依据或"致命伤"。裁定HOLD时不能以"ADX<20"为首要理由
- 🔴 ADX=17.1的正确解读：趋势启动早期，应重点关注DC20/DC55通道突破确认信号和成交量配合，而非据此判定"不做"
- ✅ 裁决reasoning中ADX提及占比不得超过总论证篇幅的1/3
- ✅ 排名/评分不能以ADX为首要因子——通道突破信号（DC20/DC55/布林带）才是主线
- ✅ 六维评分中的"趋势结构"维度应将ADX作为通道突破的辅助验证，而非独立主导评分
...
┌─────────────────────────────────────────────────────────────┐
│                     闫判官原始素材包                          │
│  (1)通道突破信号 (2)L1-L4指标 (3)factor_timing (4)链证源    │
...
└──────────────┬──────────────┬──────────────┬────────────────┘
     ...
     │ (4类字段)   │   │ (5类字段)   │   │ (4类字段)     │
     ├─────────────┤   ├────────────┤   ├──────────────┤
     │ (4)链证源   │   │ (1)通道突破 │   │ (3)基本面状态  │
     │ (5)技术面    │   │ (5)技术面   │   │ (7)事件日历   │
     │ (6)基本面    │   │ (6)基本面   │   │ (8)PnL历史   │
     │ (7)事件日历  │   │ (7)事件日历 │   │ 风控政策(内载)│
     │             │   │ (8)PnL历史  │   │ (精简)通道突破 │
     └─────────────┘   └────────────┘   └──────────────┘
```
...
2. **策执远** → 只发通道突破信号 + 技术面 + 基本面 + 事件日历 + PnL历史。不发送链证源快照
...
4. **中立约束**：周期发现是客观数据层结果，`best_period` 仅供辩手/策执远/风控明参考——**不得**在研究员中立层（多头/空头）内生成或解读，否则违反研究员中立铁律。
