"""纯 numpy/pandas 技术指标计算模块。

从 futures_data_core.indicators.legacy_numpy 迁移，去除所有外部桥接依赖
（tdx_compat / TQ-Local / technical-indicator-calc），仅依赖 numpy + pandas。
"""
...
def compute_indicators(klines, symbol: str = None, period: str = "daily") -> dict:
    """纯 numpy/pandas 计算全部技术指标。
    ...
    n = len(c)
    if n < 60:
        logger.warning("K线长度不足60，跳过指标计算 (n=%d)", n)
        return tech
    ...
    # ---- KC / CHANDELIER / SAR ----
    # 纯 numpy 版直接设为默认值（无外部 tdx_compat 依赖）
    tech["KC_UPPER"] = 0.0
    tech["KC_LOWER"] = 0.0
    tech["KC_MID"] = 0.0
    tech["CHANDELIER_LONG"] = 0.0
    tech["CHANDELIER_SHORT"] = 0.0
    tech["SAR"] = 0.0
    tech["SAR_TREND"] = 0
    ...
    # ── 数据源标记（纯 numpy 版，无外部桥接） ──
    tech["_tdx_patched"] = False
    tech["_tdx_fields"] = []
    tech["_tdx_note"] = "纯numpy计算(无TDX桥接)"
