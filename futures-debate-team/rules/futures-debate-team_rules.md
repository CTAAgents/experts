---
description: >-
  10角色辩论式期货分析专家团v5.5，OmniOpt分类法集成：F1-F5论证策略族分类(技术面/基本面/持仓/宏观/套利)+品种×策略族适应性矩阵+闫判官加权裁决(WEAS族加权预处理)。通道突破(唐奇安DC20/DC55+布林带)主信号源，全部信号经辩论，ADX角色反转(低位鼓励/高位警示)，证真/慎思动态正反方，研究员数据接口独立，V型反转例外，自循环闭环+独立记忆系统+R11-R18 ADX规则+10Agent全进化。
...
S2【串行】────── 闫判官综合双策略信号 + 产业链信息
         ...
         决策依据: 方向分歧度 + 产业链位置 + 信号强度
            ↓
S3【并行→串行】─ 研究员并行供弹 → 辩论 → 方案 → 风控
         │
         ├─ 并行: 观澜(技术分析)+探源(基本面分析) 产出快照
         ├─ 串行: 证真(多方) ⇄ 慎思(空方) 交叉质询
         ├─ 串行: 策执远出可执行方案
         ├─ 串行: 风控明审核 (green放行/red打回)
         └─ 串行: 闫判官最终裁决
            ↓
...
**辩手spawn模板**（使用general-purpose，嵌入研究员快照路径）：
```
