...
| 指标 | 值 | 说明 |
|:-----|:---|:-----|
| 当前成熟度 | 4.0/5.0 | Phase 1（G1-G4）已完成 |
| 已完成差距 | 4 (G1 schema, G2 trace, G3 日志, G4 版本) | 2026-07-10 修复 |
| 剩余差距 | 11 (5 P1 + 6 P2) | — |
...
```
Phase 1 ──── ✅ 4.0 ──── 2026-07-10
Phase 2 ──── 🎯 4.3 ──── G7→G5→G6→G8
Phase 3 ──── 🎯 4.5 ──── G9,G10,G13 + G14
Phase 4 ──── 🎯 4.7 ──── G11,G12,G15 (长期)
```
...
| 风险 | 可能性 | 影响 | 缓解 |
|:-----|:------|:-----|:-----|
| pipeline 测试因硬编码路径在 Windows 上失败 | 中 | 阻塞 | 测试中使用临时目录 + monkeypatch |
| scheduler 触发逻辑依赖 datetime.now() 难以 mock | 中 | 低 | 注入 `now_fn` 可调用对象 |
| memory_writer 并发测试需要多线程 | 低 | 中 | 使用 threading.Thread + Queue |
| G14 迁移逻辑复杂度超预估 | 中 | 低 | 先做直链迁移（单版本跳转），暂不做图搜索 |
