...
> 完整方案详见 [`FUTURE_ARCHITECTURE.md`](FUTURE_ARCHITECTURE.md)

当前系统基于 WorkBuddy 平台以日频运行。远期规划演进为**四层独立部署**的分布式架构，支撑 **1min/5min 级别** 的中高频交易：

| 层次 | 职责 | 技术栈 |
|:-----|:-----|:-------|
| **实时数据层** | Tick/K线流式采集，品种分区shard | Kafka + 独立Pod |
| **信号计算层** | GPU加速指标计算，增量递推免全量重算 | CuPy + Numba + ONNX |
| **Agent决策层** | 多Agent通过Kafka消息驱动异步并行 | CrewAI + K8s HPA |
| **执行层** | 微秒级订单路由，对接CTP柜台 | Rust/C++ |

**三阶段路线**：当前 daily → 阶段一微服务化(4h/1h) → 阶段二Agent异步化(15min) → 阶段三高频适配(5min/1min)
...
| 文件 | 用途 | 写入者 |
|:----|:----|:------|
| **v4.6.2** | **2026-07-06** | **自动发布**：流水线完成后自动版本号自增、文档同步、GitHub推送 |
