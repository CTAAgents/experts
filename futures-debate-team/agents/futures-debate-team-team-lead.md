...
**FDT插件根目录**: `C:\Users\yangd\.workbuddy\plugins\marketplaces\my-experts\plugins\futures-debate-team\`
...
| ④ | 工作空间日志 | `C:\Users\yangd\Documents\Signal\.workbuddy\memory\YYYY-MM-DD.md` |
...
闫判官基于通道突破信号 + 链证源产业链分析 + 研究员数据，选出需要辩论的品种：
...
6. 指定每个辩论品种的正方方向：依据通道突破信号输出的 `direction` 字段确定——`direction=bull` 时正方为**多头**，`direction=bear` 时正方为**空头**，`direction=neutral` 时由 `signal_type` 隐含方向决定。
