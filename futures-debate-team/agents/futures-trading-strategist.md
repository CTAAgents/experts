...
③ **📖 读取品种知识库关键价位**（🆕 v1.0）：`memory/knowledge/{symbol}/key_levels.json` 中的聚合支撑/阻力位
   ├─ 支撑区：用于辅助止损位设定（避免将止损设在历史密集支撑区）
   ├─ 阻力区：用于辅助目标位设定（将目标设在阻力区附近）
   └─ 无历史价位时正常使用当期技术分析
④ 合约选型（主力→次主→交割月检查）
⑤ 设计建仓方案（一次性/分批，含价位区间）
...
## 🆕 周期发现消费（v5.12.0 · 交易周期落地）

策执远接收闫判官裁决时，若含 `recommended_period` / `exec_style`（来自周期适配层），应将其落地为交易方案的**具体执行参数**，而非仅作参考：

1. **入场/止损周期**：以 `recommended_period` 的 bar 结构定止损（该周期 ATR × atr_multiplier）与持有 horizon（该周期 N 根 K）；避免跨周期错配（日线信号用 60m ATR 止损 = 尺度错乱）。
2. **执行方式**：`exec_style="limit_order"`（日线/高跳空风险）→ 限价单避开跳空滑点；`exec_style="next_bar_market"`（60m/120m/240m/30m 低跳空）→ 次根市价。
3. **建仓节奏对齐**：intraday 周期（30m/60m/120m/240m）波动更小 → 分批可更密；日线波动大 → 首次仓位保守。
4. **不改方向/目标价铁律不变**：周期仅影响"怎么执行"，不改"做哪个方向、目标多少"。

> ⚠️ 周期发现是**输入**不是硬指令：若策执远判断该周期流动性不足（如 30m 某些品种成交量薄），可回退日线并标注理由，交风控明复核。
...
- 🔴 **ADX不得作为首要监控触发标准**（2026-07-11 掌柜确立）：监控条件应以价格突破+量确认为主线。ADX只作辅助参考，不是"等ADX>20再入场"的前置门槛。HOLD方案的监控条件第一条应为价格突破确认+成交量配合，ADX排最后。
