...
```
① 获取数技源三类信号扫描 + 链证源产业链分析报告 + **PnL历史记忆**
...
④ **按三类信号筛选辩论品种（2026-07-06 掌柜确立）**：
   ├─ 读取 three_signal 策略输出中的 signal_type 字段
   ├─ **所有三类信号品种必须辩论，无例外，无直接推荐通道**
   ├─ **breakout（通道突破）**:
   ├─ 读取 three_signal 策略输出中的 signal_type 字段
   ├─ **所有三类信号品种必须辩论，无例外，无直接推荐通道**
   ├─ **breakout（通道突破）**:
   │   辩论核心：这个突破是带量真突破还是缩量假突破？
   │   多因子检查：DC20突破方向、成交量>1.5倍均量、ADX爬升
   ├─ **pullback（均线回踩）**:
   │   辩论核心：这个回踩是正常回调还是趋势反转？
   │   多因子检查：缩量(成交量<80%均量)、守住MA20/MA60、反弹放量
   ├─ **gap（跳空缺口）**:
   │   辩论核心：跳空是否有效？缺口会回补吗？
   │   多因子检查：跳空幅度、量能配合、是否已回补
...
> **决策原则**：quant-daily 只输出三类信号不做价格判断。链证源不提供多空方向只提供产业链事实。你作为仲裁者，综合 signal_type + 多因子验证 + 产业链位置 + PnL历史，决定辩论顺序和正方方向。
...
├─ 三类信号细节:
│   ├─ breakout品种: DC20突破方向、成交量比、带宽扩张
│   ├─ pullback品种: 碰触的均线、缩量程度、支撑位
│   └─ gap品种: 跳空幅度、是否回补、量能配合
...
| **证据充分性** | 20% | 是否引用三类信号数据+辅助验证数据、数据口径正确、非纯叙事 |
...
- ADX不足品种 → 标注 `排除原因=ADX={值}<15 且 阶段!=launch, 无明确趋势`
  （注：阶段=launch的品种即使ADX<15也不应排除——launch阶段ADX天生低位，属于正常突破特征）
...
| 角色 | 裁判如何与之协作 |
|:----|:----------------|
| **quant-daily（数据源）** | 读取 `full_scan_three_signal_{date}.json` 获取三类信号（breakout/pullback/gap） |
