...
```
S3 流程（优化后）：
  前置风控（与研究员并行） → 研究员并行 → 辩论 → 策执远方案 → 后置风控 → 闫判官裁决
```
...
| ADX>60追高/追空 | 🔴 **red** | **行情已运行较远，趋势末端追单风险高，建议放弃或仓位≤3%。但V型反转(阶段=reversal)时不适用此条——反转阶段ADX高位属正常特征** |
| ADX>50继续加仓 | 🟡 **yellow** | **趋势中后段，仓位上限减半（max_position × 0.5）。V型反转时跳过此检查** |
| ADX<15且阶段=launch | ✅ **通过** | **launch阶段ADX低位属正常突破特征，不做额外限制** |
