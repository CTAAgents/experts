# 期货交易辩论专家团 — 用户使用手册 (v4.5)

## 1. 概述

期货交易辩论专家团是一个 **多Agent深度辩论型期货分析系统**，通过 **11个专业角色Agent** 协作，对商品期货品种进行结构化多空辩论分析。

**核心理念**：quant-daily 只输出原始信号数值不做判断 → debate_brief.py 进行双通道分离 → 链证源先于闫判官做产业链分析 → 闫判官自主决定辩论品种与方向（同时为直接推荐品种手动设定交易参数）→ 多方和空方从研究员资料中提取证据进行辩论 → 策执远出方案（支持双通道输入）→ 风控明审核 → 明鉴秋合并双通道输出。

**版本**：v4.5 | **Agent数**：11（1协调员 + 10角色）| **双策略**：L1-L4 + factor_timing

**v4.5 亮点**：
- **双通道分离**：共识+launch+非极端 → 直接推荐（免辩论）；分歧/极端/链补 → 完整辩论流程
- **分歧品种信号强度门槛**：分歧入池需至少一边信号 >= MODERATE，避免为辩论而辩论
- **极端分歧左侧反转标记**：RSI极端+分歧时标记左侧反转机会，正方=逆转方向
- **闫判官直接参数设定**：基于 price/ATR/ADX 手动设定入场/止损/目标，由策执远校验
