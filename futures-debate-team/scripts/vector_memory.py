...
class VectorMemory:
    ...
    def query(self, symbol: str, regime: str = None, signal_fingerprint: str = None, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        RAG检索：查找相似的历史交易记录。
    ...
    def add_attribution_tags(self, record_id: str, tags: Dict[str, str]) -> bool:
        ...
        with sqlite3.connect(str(self.mid_db)) as conn:
            conn.execute(
                "UPDATE trades SET attribution_tags = ? WHERE id = ?",
...
if __name__ == "__main__":
    ...
    for r in results:
        rec = r["record"]
        print(f"  {rec['symbol']} {rec['direction']} PnL={rec['pnl']} score={r['similarity_score']:.2f} layer={r['layer']}")
