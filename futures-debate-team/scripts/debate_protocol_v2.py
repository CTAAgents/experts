...
class DebateProtocolV2:
    ...
    def run_debate(
        ...
        # ── 交叉质证轮（Round 2）──
        round2 = self._round_2_cross_examination(
            round1["affirmative_args"], round1["opposition_args"], evidence_data
        )
        
        # 分歧度>0.7 → 追加深度辩论（Round 3）
        round3 = None
        if divergence > 0.7:
            round3 = self._round_3_deep_debate(
                round2["affirmative_args"], round2["opposition_args"], evidence_data
            )
        else:
            round3 = self._round_3_response(
                round2["affirmative_args"], round2["opposition_args"], evidence_data
            )
        
        # 计算最终得分
        final_scores = self._calculate_final_scores(
            round1, round2, round3, divergence
        )
        
        return self._finalize_with_rounds(round1, round2, round3, final_scores, divergence)
    
    def _round_1_opening(
        self, aff_args: List[Dict], opp_args: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _generate_attacks(
        self, target_args: List[Dict], attacker: str, evidence_data: Dict
    ) -> List[Dict[str, Any]]:
        """生成攻击列表。"""
        attacks = []
...
if __name__ == "__main__":
    ...
    aff_args = [
        {"id": "a1", "text": "RB库存连续下降", "confidence": 0.8, "data_age_days": 2, "source_type": "exchange", "regime": "trend"},
        {"id": "a2", "text": "基差走强", "confidence": 0.7, "data_age_days": 1, "source_type": "wind", "regime": "chain"},
    ]
    opp_args = [
        {"id": "o1", "text": "需求季节性走弱", "confidence": 0.6, "data_age_days": 5, "source_type": "news", "regime": "fundamental"},
        {"id": "o2", "text": "宏观数据不及预期", "confidence": 0.5, "data_age_days": 3, "source_type": "news", "regime": "macro"},
    ]
