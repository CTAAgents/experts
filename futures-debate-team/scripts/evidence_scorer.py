...
def score_debate(bull_output: dict, bear_output: dict) -> Dict[str, Any]:
    ...
    # 综合评分
    bull_total = bull_avg * 0.4 + min(bull_high * 0.1, 0.2) + (bull_with_evidence / max(len(bull_claims), 1)) * 0.2 + (bull_rebuttals / max(len(bull_claims), 1)) * 0.1 + (bull_fallacies / max(len(bull_claims), 1)) * 0.1
    bear_total = bear_avg * 0.4 + min(bear_high * 0.1, 0.2) + (bear_with_evidence / max(len(bear_claims), 1)) * 0.2 + (bear_rebuttals / max(len(bear_claims), 1)) * 0.1 + (bear_fallacies / max(len(bear_claims), 1)) * 0.1
...
if __name__ == "__main__":
    ...
    bull = {
        "evidence": {
            "technical": [
                {"claim_id": "证真-D1", "point": "螺纹趋势向上", 
                 "evidence_value": "ADX=28", "evidence_source": "L1-L4策略",
                 "evidence_date": "2026-07-05", "impact_level": "HIGH"},
                {"claim_id": "证真-D2", "point": "成交量放大确认",
                 "evidence_value": "成交量+15%", "evidence_source": "交易所",
                 "evidence_date": "2026-07-04", "impact_level": "MEDIUM"},
            ],
            "fundamental": [
                {"claim_id": "证真-D3", "point": "库存低位",
                 "evidence_value": "社库352万吨", "evidence_source": "Mysteel",
                 "evidence_date": "2026-07-03", "impact_level": "HIGH",
                 "rebuttal_to": "慎思-D1", "logical_fallacy": "数据过时"},
            ],
            "chain": [],
        }
    }
    bear = {
        "evidence": {
            "technical": [
                {"claim_id": "慎思-D1", "point": "RSI超买",
                 "evidence_value": "RSI=72", "evidence_source": "L1-L4策略",
                 "evidence_date": "2026-07-05", "impact_level": "HIGH"},
            ],
            "fundamental": [],
            "chain": [],
