...
def _build_market_data(tech_list: list[dict], df_map: Optional[dict] = None) -> Dict[str, Dict]:
    ...
    for tech in tech_list:
        ...
        if dc20_break == "up" or ma_align == "bull":
            far_close_est = close * 0.995
        elif dc20_break == "down" or ma_align == "bear":
            far_close_est = close * 1.005
        else:
            far_close_est = close * 1.002
...
def _generate_factor_timing_scan(
    ...
    """
    因子择时策略：输出 full_scan_{YYYYMMDD}.json 格式的多空信号。
    """
    ...
    if CONFIG.get("sector_neutral", False):
        ...
        for sector in set(sym_to_sector.values()):
            ...
            if len(sector_syms) < 3:
                ...
                s_mean = sector_df.mean()
                s_std = sector_df.std()
                for s in sector_syms:
                    df_z.loc[s] = (df.loc[s] - s_mean) / (s_std + CONFIG["epsilon"])
        df = df_z
    else:
