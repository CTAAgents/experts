...
def _svg_radar_chart(
    ...
    for i in range(n):
        x, y = _polar(i, 100)
        axes += (
            f'<line x1="{cx}" y1="{cy}" x2="{x:.1f}" y2="{y:.1f}" stroke="rgba(255,255,255,0.08)" stroke-width="1"/>'
        )
        lx = x + (x - cx) * 0.15
        ly = y + (y - cy) * 0.15
...
def generate_markdown_report(
    ...
    for cn, cd in chain_results.items():
        ...
        for m in cd["members"]:
            lines.append(f"| {m['pid']} | {m['price']:.2f} | {m['score']:.0f} | {m['trend']} | {m['oi']:,} |")
        lines.extend(["", "---", ""])
...
def generate_debate_html_report(truth: dict) -> str:
    ...
    def _card(s: dict, cls: str, rank_label: str) -> str:
        ...
        style_tags = "+".join(
            f'<span style="color:{_style_colors.get(t, "#94a3b8")}">{t}</span>' for t in style_raw.split("+")
        )
...
<div class="db"><h4>📢 辩论论点</h4>{pro_html}{con_html}</div>
        ...
        [
            {
                "r": r["rank"],
                "s": r["symbol"],
                "a": r.get("adjusted_rank", 0),
                "v": r.get("veto_penalty", 1),
    ...
    short_radar_svg = (
        ...
        if short_radar_data
        else '<div style="color:#555;text-align:center;padding:40px">无做空信号</div>'
    )
    long_radar_svg = (
        _svg_radar_chart(
            long_radar_data,
        ...
        if long_radar_data
        else '<div style="color:#555;text-align:center;padding:40px">无做多信号</div>'
    )
    rank_bar_svg_rows = "".join(_svg_rank_row(r["symbol"], r.get("rank", 0), r.get("adjusted_rank", 0)) for r in ranked)
