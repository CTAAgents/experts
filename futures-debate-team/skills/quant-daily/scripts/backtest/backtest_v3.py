...
def calc_sharpe(returns: List[float], rf: float = 0.02) -> float:
    ...
    excess = r - rf / 252
    if np.std(r, ddof=1) < 1e-8: return 0.0
...
def compute_l1l4_score(tech: dict, close_arr: List[float], start: int) -> dict:
    ...
    if all(v is not None for v in [ma5, ma10, ma20]):
        if ma5 > ma10 > ma20: l1_score += 10
        elif ma5 < ma10 < ma20: l1_score -= 10
    ...
    if rsi > 55: l4_score += 3
    elif rsi < 45: l4_score -= 3

    veto = 0
    if adx < 15: veto -= 10
    if rsi > 80 or rsi < 20: veto -= 8
    ...
    f1_score = 0
    if roc10 > 0: f1_score += 1
    else: f1_score -= 1
    if price > ma20 and ma20 < ma60: f1_score += 1  # 短期向上且长期未转势
    if price < ma20 and ma20 > ma60: f1_score -= 1  # 短期向下且长期已转势

    # F2: 展期/carry (term structure proxy) — 近远月价差
    f2_score = 0
    if st_dir == 1: f2_score += 1  # supertrend多头=backwardation傾向
    elif st_dir == -1: f2_score -= 1

    # F3: 偏度 (skew) — RSI极端位置
    f3_score = 0
    if rsi > 60: f3_score += 1
    elif rsi < 40: f3_score -= 1
    if cci > 100: f3_score += 1
    elif cci < -100: f3_score -= 1

    # F4: 量价相关性 (price-volume)
    f4_score = 0
    if vol_ratio > 1.2 and l1_score > 0: f4_score += 1  # 放量上涨
    elif vol_ratio > 1.2 and l1_score < 0: f4_score -= 1  # 放量下跌
    if obv and obv_ma and obv > obv_ma: f4_score += 1
    elif obv and obv_ma and obv < obv_ma: f4_score -= 1

    # F5: 逆情绪 (contrarian) — CCI极端 / ST方向
    f5_score = 0
    if cci > 200: f5_score -= 1  # 极度超买→逆空
    elif cci < -200: f5_score += 1  # 极度超卖→逆多
    ...
    return {
        'l1l4_total': total, 'l1l4_dir': direction, 'l1l4_grade': grade,
        'l1': l1_score, 'l2': l2_score, 'l3': l3_score, 'l4': l4_score,
        'f_direction': f_direction, 'f_net_vote': net_vote, 'f_confidence': f_confidence,
        'price': price, 'adx': round(adx, 1), 'rsi': round(rsi, 1),
...
def run_non_overlap_backtest(symbol: str, kline: List[dict],
                              forward: int = 10, min_start: int = 60,
    ...
    while pos + forward < n:
        ...
        # factor_timing信号
        f_signal = 'BUY' if f_dir == 'bull' and f_conf >= 0.2 else ('SELL' if f_dir == 'bear' and f_conf >= 0.2 else 'HOLD')
        ...
        sig = {
            'date_idx': pos,
            'price': entry_price,
            'l1l4_dir': l_dir, 'l1l4_grade': l_grade,
            'f_dir': f_dir, 'f_confidence': f_conf,
        ...
        if open_trade:
            ...
            if HAVE_JOURNAL:
                try:
                    tid = record_trade(symbol, 'long' if trade_side == 'BUY' else 'short',
                                       entry_price, entry_price * 0.95 if trade_side == 'BUY' else entry_price * 1.05,
                    ...
                    direction_ok = (ret > 0)
                    annotate_prediction(tid, l1_signal if l1_signal != 'HOLD' else 'HOLD',
                                        'BUY' if direction_ok else 'SELL', ret)
                except Exception:
                    pass
    ...
    return {
        ...
        'dual_signal_breakdown': {
            ...
            'dual_win_rate': round(sum(1 for t in trades if t.get('dual') and t['net_return'] > 0) / max(sum(1 for t in trades if t.get('dual')), 1) * 100, 1),
            'l1_only_signals': sum(1 for t in trades if not t.get('dual')),
            'l1_win_rate': round(sum(1 for t in trades if not t.get('dual') and t['net_return'] > 0) / max(sum(1 for t in trades if not t.get('dual')), 1) * 100, 1),
        },
    }
...
def adjust_weights(trades: List[Dict], results: List[Dict]) -> Dict:
    ...
    """
    if len(trades) < 10:
        return {'note': '样本不足(需要10+交易)', 'l_weights': [35, 35, 20, 10],
                'f_threshold': 0.2, 'adx_min': 25}
...
def main():
    ...
    parser.add_argument('--symbols', '-s', default='RB,HC,PK',
                        help='品种列表(逗号分隔)')
    ...
    for sym in symbols:
        ...
        m = report.get('metrics', {})
        db = report.get('dual_signal_breakdown', {})
        print(f"  {m.get('total_trades',0)}笔 | 双策略{db.get('total_dual_signals',0)}笔(胜率{db.get('dual_win_rate',0)}%)"
              f" | L1-L4{db.get('l1_only_signals',0)}笔(胜率{db.get('l1_win_rate',0)}%)")
        print(f"  CR={m.get('cumulative_return',0):+.2f}% SR={m.get('sharpe_ratio',0):.2f} 胜率={m.get('win_rate',0):.1f}%"
