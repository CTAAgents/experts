...
if not os.path.isdir(PARENT_DIR):
    PARENT_DIR = r'C:\Users\yangd\.workbuddy\skills'
...
def strategy_rsi(observations: List[dict], forward: int = 10,
...
def benchmark_monte_carlo(returns: List[float], n_iterations: int = 2000) -> Dict:
    ...
    if len(returns) < 5:
        return {'avg_win_rate': 0, 'p95_win_rate': 0, 'p_value': 1.0,
                'is_significant': False, 'n_iterations': n_iterations}
...
def run_symbol_backtest(symbol: str, name: str = '',
                         days: int = 250, step: int = 5,
                         min_start: int = 60, forward_days: List[int] = None,
    ...
    for start in range(min_start, n - max(forward_days), step):
        ...
        ma20 = tech.get('MA20', np.mean(closes[start-19:start+1]) if start>=19 else np.mean(closes[:start+1]))
        ...
        l1_score = 0
        # 价格 vs MA20/MA60
        if price > ma60: l1_score += 20
        elif price < ma60: l1_score -= 20
        if price > ma20: l1_score += 10
        elif price < ma20: l1_score -= 10
        # MA排列
        ...
        # L4: MACD + RSI确认（权重10）
        l4_score = 0
        if macd_dif > 0: l4_score += 5
        else: l4_score -= 5
        if rsi_val > 55: l4_score += 3
        elif rsi_val < 45: l4_score -= 3

        # 否决分：ADX极低、RSI极端、超买超卖
        veto = 0
        if adx_val < 15: veto -= 10  # 无趋势
        if rsi_val > 80 or rsi_val < 20: veto -= 8  # 极端RSI
        ...
        ob = {
            'sym': symbol, 'name': name or symbol,
            'date_idx': start, 'price': price,
            'total': total, 'direction': direction, 'grade': grade,
            'adx': round(adx_val, 1), 'rsi': round(rsi_val, 1),
            'ma_cross_buy': ma_cross_buy,
            'rsi_buy': rsi_buy, 'rsi_sell': rsi_sell,
            'atr': round(atr_val, 2),
            'volume_ratio': round(vol_ratio, 2),
            'l1': l1_score, 'l2': l2_score, 'l3': l3_score, 'l4': l4_score,
        }
        # 双策略共识：L1-L4 + factor的共振
        ob['dual_consensus_sell'] = (direction == 'SELL' and grade in ('WEAK', 'WATCH', 'STRONG')
                                     and ob['adx'] > 25)  # 趋势过滤
        ob['dual_consensus_buy'] = (direction == 'BUY' and grade in ('WEAK', 'WATCH', 'STRONG')
                                    and ob['adx'] > 25)
    ...
    if not observations:
        return {'symbol': symbol, 'name': name, 'error': '无有效观测',
                'status': 'ERROR'}
    ...
    return {
        ...
        'config': {
            'days': days, 'step': step, 'min_start': min_start,
            'forward_days': forward_days, 'mc_iterations': mc_iterations,
            'kline_bars': n, 'observations': len(observations),
        },
...
<h1>📊 回测报告 — {symbol}</h1>
<div class="sub">{name} | {ts} | {report.get("config",{}).get("kline_bars","?")}根K线 | 截面: {report.get("config",{}).get("observations","?")}</div>

<div class="card">
    <h2>📈 核心指标总览</h2>
    <div class="summary-grid">
        <div class="s-item {"positive" if report.get("strategies",{}).get("watch_buy_10d",{}).get("cumulative_return",0) > 0 else "negative"}">
            <div class="v">{fmt_num(report.get("strategies",{}).get("watch_buy_10d",{}).get("cumulative_return",0))}%</div>
            <div class="l">累计收益 CR</div>
        </div>
        <div class="s-item {"positive" if report.get("strategies",{}).get("watch_buy_10d",{}).get("sharpe_ratio",0) > 1 else "neutral"}">
            <div class="v">{fmt_num(report.get("strategies",{}).get("watch_buy_10d",{}).get("sharpe_ratio",0))}</div>
            <div class="l">夏普 SR</div>
        </div>
        <div class="s-item {"positive" if report.get("strategies",{}).get("watch_buy_10d",{}).get("max_drawdown",0) < 10 else "negative"}">
        ...
        <div class="s-item neutral">
            <div class="v">{fmt_num(report.get("strategies",{}).get("watch_buy_10d",{}).get("profit_factor",0))}</div>
            <div class="l">盈亏比</div>
        </div>
    </div>
    {comparison}
...
<div class="card">
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap">
        {"".join(f'''<div style="background:#12141a;border-radius:6px;padding:10px 14px;text-align:center;min-width:80px">
        ...
        </div>''' for g in ['STRONG','WATCH','WEAK','NOISE'])}
    </div>
</div>
...
<div class="card">
    ...
    <table style="width:auto">
        ...
        <tr><td style="font-weight:600">持有周期</td><td>{", ".join(f"{fd}日" for fd in report.get("config",{}).get("forward_days",[]))}</td></tr>
        <tr><td style="font-weight:600">蒙提卡罗</td><td>{mc.get("n_iterations",report.get("config",{}).get("mc_iterations","?"))}次</td></tr>
...
def main():
    ...
    for sym in symbols_raw:
        ...
        print(f"     WATCH+BUY: {wb.get('signals',0)}次 胜率{wb.get('win_rate',0):.1f}% CR{wb.get('cumulative_return',0):+.2f}% SR{wb.get('sharpe_ratio',0):.2f}")
        print(f"     WEAK+SELL: {ws.get('signals',0)}次 胜率{ws.get('win_rate',0):.1f}% CR{ws.get('cumulative_return',0):+.2f}% SR{ws.get('sharpe_ratio',0):.2f}{fee_tag}")
