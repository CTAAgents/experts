...
def score_l1l4(tech: dict, price: float) -> dict:
    ...
    l1 = (20 if price > ma60 else -20 if price < ma60 else 0) + \
         (10 if price > ma20 else -10 if price < ma20 else 0)
    if all(v for v in [ma5, ma10, ma20]):
        l1 += 10 if ma5 > ma10 > ma20 else (-10 if ma5 < ma10 < ma20 else 0)

    l2 = 0
    if vol_ratio > 1.5: l2 = 15 if l1 > 0 else -15
    elif vol_ratio > 1.2: l2 = 5 if l1 > 0 else -5

    l3 = 0
    if adx > 25:
        f = min(adx / 100, 0.5)
        if pdi > mdi: l3 = int(10 * f * 2)
        else: l3 = -int(10 * f * 2)
    ...
    return {
        ...
        "l1l4_total": total,
        "l1": l1, "l2": l2, "l3": l3, "l4": l4, "veto": veto,
    }
...
def extract_features(tech: dict, price: float, closes: List[float],
...
def collect_training_data(kline: List[dict], symbol: str,
                           forward: int = 10, min_start: int = 60) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """从K线数据中提取训练特征和标签。"""
...
def run_ml_backtest(symbol: str, kline: List[dict],
                     model: DirectionClassifier,
    ...
    while pos + forward < n:
        ...
        # 方向映射
        signal = 'HOLD'
        if ens_dir > 0 and adx > 25: signal = 'BUY'
        elif ens_dir < 0 and adx > 25: signal = 'SELL'

        # 仅规则层信号（用于对比）
        rule_dir = rule_out.get('direction', 0)
        rule_signal = 'HOLD'
        if rule_dir > 0: rule_signal = 'BUY'
        elif rule_dir < 0: rule_signal = 'SELL'

        sig = {
            'date_idx': pos, 'price': price,
            'ensemble': ensemble_out,
            'rule': rule_out,
            'ml': ml_out,
            'signal': signal, 'rule_signal': rule_signal,
            'adx': round(adx, 1),
        }
    ...
    return {
        ...
        'config': {'forward_days': forward, 'min_start': min_start,
                    'bars': n, 'fee_rate': fee_rate, 'model': 'ML+规则集成'},
        'trades': len(trades),
        ...
        'ml_stats': {
            'signal_agreement': round(sum(1 for s in all_signals
                                          if s['signal'] != 'HOLD') / max(len(all_signals), 1) * 100, 1),
...
def main():
    ...
    for sym in symbols:
        ...
        print(f"\n[{sym}] ML回测...")
        t0 = time.time()
        report = run_ml_backtest(sym, kline, model, ensemble,
                                  forward=args.forward, fee_rate=args.fee_rate)
        ...
        m = report['metrics']
        ms = report['ml_stats']
        print(f"  {m['total_trades']}笔 | 胜率{m['win_rate']}% | CR={m['cumulative_return']:+.2f}% | SR={m['sharpe_ratio']:.2f}")
