...
agent_created: true
changelog: |
  v3.8.0 (2026-07-11): ⚡ 时延优化四步实施完成 — ①P2研究员并行spawn(串行→并行,省~20min) ②信号分级驱动辩论深度(C1快速裁决/C2两轮/C3四轮/C4不辩) ③闫判官时段表改为等级自适应 ④盘前预计算缓存(scan_all盘中免跑,省~5min)。加权平均耗时从60min→~10min(C1场景8min,C2场景15min,命中缓存后约6min)。新增自动化「FDT盘前预计算缓存刷新」每日08:00自动运行。
...
### 🆕 预计算缓存路由（2026-07-11 新增·时延优化 Step 4）

明鉴秋在进入 P1 前，**先检查预计算缓存**：

```python
from scripts.precompute_cache import load_cache, build_cache, refresh_prices_for_signal, check_event_invalidation

# 检查是否有重大事件（FOMC/非农等）→ 全量扫描
if check_event_invalidation():
    state["cache_hit"] = False
    state["cache_reason"] = "major_event"
else:
    cached_data, is_fresh = load_cache()
    state["cache_hit"] = is_fresh
    state["cached_data"] = cached_data if is_fresh else None

if state["cache_hit"]:
    # ── 缓存命中：跳过 P1 scan_all 全流程，仅增量更新 ──
    # 从缓存中加载数据
    result = state["cached_data"]
    # 仅刷新辩论品种的最新价格（~1min）
    debate_symbols = state.get("debate_symbols", [])
    if debate_symbols:
        result = refresh_prices_for_symbols(result, debate_symbols)
    state["data_raw"] = result
    state["debate_mode"] = "from_cache"
    print(f"[预计算] ✅ 缓存命中，跳过 P1 scan_all（{cache_info['remaining_hours']}h 内有效）")
else:
    # ── 缓存未命中：执行标准 P1 scan_all 全流程 ──
    state["debate_mode"] = "full"
    print("[预计算] 缓存未命中，执行全量扫描")
```

**缓存的好处**：
- 盘前（08:00）预跑一次 scan_all（~5min），盘中触发辩论时无需再等 P1
- 盘中仅需刷新特定品种最新价格（~1min）
- 缓存有效期 4h，重大事件日自动失效
- C2/C3 场景额外节省 ~5min
...
> 若 `state["cache_hit"] = True`，跳过本节直接进入 P2。
