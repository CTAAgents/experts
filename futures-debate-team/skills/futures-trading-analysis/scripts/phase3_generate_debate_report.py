...
if rps:
    ...
    parts.append(f"""<div class="yellow-box"><strong>动态冗余检测：</strong>{'|'.join(f'{rp["chain"]}: {rp["redundant"]}→{rp["primary"]}' for rp in rps)}</div>""")
...
# Top5
top5_info = {
    'PF':('短纤',6908,-62,-14,75.9,'聚酯链唯一代表，双策略共振。ADX极强+成本坍塌。','85%','6400-6500','7300','2.1:1'),
    'RB':('螺纹钢',3062,-70,0,69.2,'黑色系最强信号。淡季效应+钢厂亏损，强趋势空头。','78%','2850-2900','3200','1.7:1'),
    'BU':('沥青',3851,-60,-2,63.5,'能源链最佳。需求疲弱主导，低库存提供安全垫。','72%','3500-3600','4050','1.6:1'),
    'SM':('锰硅',5832,-63,0,62.5,'铁合金独立品种。供需宽松主导，不受黑色系限制。','70%','5400-5500','6100','1.6:1'),
    'SA':('纯碱',1103,-61,2,49.1,'建材唯一代表。弱需求拖累，供增需弱压制价格。','65%','1000-1020','1180','1.5:1'),
}
