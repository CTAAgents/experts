...
for cname, cinfo in chain_results.items():
    ...
    if chain_signals:
        ...
        cinfo["avg_score"] = (
            sum(abs(s.get("confidence", 0) * 100) for s in chain_signals) / len(chain_signals) if chain_signals else 0
        )
        dc = {}
...
def build_html():
    ...
    for pid in SYMBOL_KEYS:
        ...
        if isinstance(jv, dict) and "final_direction" in jv:
            ...
            v_reason = (
                v_raw.get("reasoning", "")[:150]
                if isinstance(v_raw, dict)
                else d.get("judge_verdict", "")[:150]
                if isinstance(d.get("judge_verdict", ""), str)
                else ""
            )
        ...
        if isinstance(tp, dict) and (tp.get("options") or tp.get("plan")):
            if tp.get("options"):
                ...
                plan_summary = f"{tp.get('plan', '')} | {tp.get('entry', '')} | 仓{tp.get('position', '')} | 损{tp.get('stop_loss', '')}"[
                    :80
                ]
        else:
